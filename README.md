...
> Educational research artefact, not a clinical tool. Findings are aggregate and must not be used in individual clinical decisions.

A consolidation of publicly available UK ADHD waiting-time data, with documented data quality limitations.

**Author:** Noble Chidera Onyema
**MSc Applied AI, University of Abertay**
**Live dashboard:** [adhd-care-equity-tracker-uk-noble-chidera-onyema.streamlit.app](https://adhd-care-equity-tracker-uk-noble-chidera-onyema.streamlit.app)
...
| `notebooks/05_opensafely_and_commons_library.ipynb` | OpenSAFELY ADHD diagnosis and prescribing analysis (9-year series) + House of Commons Library briefing CBP-10551 | Complete |
...
| `app/streamlit_app.py` | Public Streamlit dashboard, five views over the harmonised data | Live |
...
Harmonised fact table reproduces every source's headline figure to the unit. The DuckDB-built `adhd_atlas_fact.parquet` reconstructs the 562,480 ADHD003 total, the OpenSAFELY 0.16%/0.92% female prevalence endpoints, and the Children's Commissioner ethnicity shares (sum to 100%). Validation queries are in `notebooks/06_harmonisation_duckdb.ipynb` and also run live on the deployed dashboard's Methodology page.
...
Data quality flags surfaced and handled in the notebooks: mixed date formats in the same column (UK `DD/MM/YYYY` and ISO `YYYY-MM-DD`), `*` suppression for small-count cells (240 cells in the MI-ADHD February 2026 release), a 10-row discrepancy between age-group and ethnicity sums of the same indicator at the same month, and messy multi-row PDF table headers in the Children's Commissioner report.
...
The 13-month MI-ADHD activity time series is too short for meaningful long-horizon forecasting. The 9-year OpenSAFELY series covers diagnosis and prescribing, not the waiting list itself.
...
Python 3.11 · pandas · DuckDB · scikit-learn · Plotly · pdfplumber · Streamlit · JupyterLab. Deployed on Streamlit Community Cloud.
...
│   ├── views/          overview, waiting_times, demographics, trends, methodology
